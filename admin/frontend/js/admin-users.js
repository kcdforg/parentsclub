...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function displayAdminUsers(adminUsers) {
        ...
        adminUsers.forEach(admin => {
            ...
            row.innerHTML = `
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    ${admin.created_by ? `Admin ${admin.created_by}` : 'System'}
                </td>
    ...
    window.toggleAdminStatus = function(adminId, activate) {
        const action = activate ? 'activate' : 'deactivate';
        if (confirm(`Are you sure you want to ${action} this admin user?`)) {
            // Implement status toggle functionality
            console.log('Toggle admin status:', adminId, activate);
        }
    };

    window.deleteAdminUser = function(adminId) {
        if (confirm('Are you sure you want to delete this admin user? This action cannot be undone.')) {
            // Implement delete admin user functionality
            console.log('Delete admin user:', adminId);
        }
    };
}); 