...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function copyAllInvitationLinks() {
        ...
        if (navigator.clipboard) {
            navigator.clipboard.writeText(allLinks).then(() => {
                ...
                copyAllLinksBtn.innerHTML = '<i class="fas fa-check"></i> All Links Copied!';
                ...
                showSuccess(`Copied ${currentInvitations.filter(inv => inv.status === 'pending').length} invitation links to clipboard!`);
            }).catch(err => {
                console.error('Failed to copy all links: ', err);
                showError('Failed to copy all links to clipboard');
            });
        } else {
            // Fallback for older browsers
            const textArea = document.createElement('textarea');
            ...
            try {
                ...
                console.error('Fallback: Oops, unable to copy all links', err);
                showError('Failed to copy all links to clipboard');
            }
            document.body.removeChild(textArea);
        }
